<?php
		if(!$_SESSION['notlogin_apply']) getLink('','', '정보수정에서 학생정보를 기입 후 다시 조회해주세요.' , '-1');
		$gid = trim($gid);
		$is_my_group = getDbRows('rb_dalkkum_applyable','group_seq='.$gid.' and sc_grade='.$_SESSION['notlogin_apply']['grade'].' and sc_class='.$_SESSION['notlogin_apply']['class'].' and sc_num='.$_SESSION['notlogin_apply']['num'].' and name="'.$_SESSION['notlogin_apply']['name'].'"');
		// 권한 검사
		...
		// 수강신청 리스트
		$myStdUid = db_query("select G.uid,G.name as group_name,SC.name as sc_name,AA.name as std_name, AA.email , AA.tel
from rb_dalkkum_applyable as AA, rb_dalkkum_apply as A, rb_dalkkum_sclist as SC, rb_dalkkum_group as G
where AA.group_seq = G.uid and AA.group_seq = A.group_seq and SC.uid = G.sc_seq and A.able_seq = AA.uid 
and AA.name = '".$_SESSION['notlogin_apply']['name']."' 
and SC.name = '".$_SESSION['notlogin_apply']['sc_name']."' 
and AA.sc_grade = ".$_SESSION['notlogin_apply']['grade']." 
and AA.sc_class = ".$_SESSION['notlogin_apply']['class']."
and AA.sc_num = ".$_SESSION['notlogin_apply']['num']."
and AA.email = '".$_SESSION['notlogin_apply']['email']."' 
and AA.tel = '".$_SESSION['notlogin_apply']['tel']."'",$DB_CONNECT);
	?>
<?php if($gid):
		$_ALIST = db_fetch_array(db_query('select A.* from rb_dalkkum_applyable as AA, rb_dalkkum_apply as A where AA.uid = A.able_seq and AA.group_seq='.$gid.' and AA.sc_grade='.$_SESSION['notlogin_apply']['grade'].' and AA.sc_class='.$_SESSION['notlogin_apply']['class'].' and AA.sc_num='.$_SESSION['notlogin_apply']['num'].' and AA.name="'.$_SESSION['notlogin_apply']['name'].'"',$DB_CONNECT));
		
		// 수강신청 취소 이용을 위한 세션 생성
		$_SESSION['notlogin_apply']['uid'] = $_ALIST['able_seq'];
		$_SESSION['notlogin_apply']['group'] = $gid;
?>
<div class="pages_join">
	...
	<div class="cl">
		<h2 class="apply fl">수강신청내역 (<?=$_Group['name']?> <?=$_SESSION['notlogin_apply']['grade']?>학년 <?=$_SESSION['notlogin_apply']['class']?>반 <?=$_SESSION['notlogin_apply']['num']?>번 <?=$_SESSION['notlogin_apply']['name']?>)</h2>
		<div class="fr">
...
$(document).ready(function(){
	...
	$('[data-act="select_group"]').on('change',function(){
		...
		location.href = "/apply/?page=mylist&gid="+select_group;
	});
});
