...
$M2 = $M3 = getUidData('rb_dalkkum_mentor',$M['memberuid']);
$hidden_array = explode('|', $M2['hiddens']);
if($M2){
...
<div id="mypage_main">
	...
	<?php if($M['mentor_confirm']=='Y' || $M['mentor_apply']): ?>
		<tr>
			<td class="key" colspan="4"><b>멘토 정보 - <font color="blue"><b><?php if($M['mentor_confirm']=='Y') : ?> 이 회원은 강사 회원입니다.<?php else: ?> 이 회원은 강사 신청자 입니다. <?php endif ?> </b></font></b></td>
		</tr>
		<tr>
			<td class="key">이름</td>
		...
		<tr>
			<td class="key">멘토 직업</td>
			<td><?php echo getJobName($M['mentor_job'])?></td>
			<td class="key"></td>
			<td></td>
		</tr>
		<?php if($M2){?>
		<tr>
			<td class="key">학력</td>
			<td colspan="3">
			<?php foreach ($M2['edu'] as $key => &$value) {
				echo $value;
				if(substr($hidden_array[0], $key,1)=='1'){
					echo "(숨김)";
				}
				echo "<br>";
				}	
		...
		<tr>
			<td class="key">경력</td>
			<td colspan="3">
			<?php foreach ($M2['career'] as $key => &$value) {
				echo $value;
				if(substr($hidden_array[1], $key,1)=='1'){
					echo "(숨김)";
				}
				echo "<br>";
				}	
		...
		<tr>
			<td class="key">자격증</td>
			<td colspan="3">
			<?php foreach ($M2['cert'] as $key => &$value) {
				echo $value;
				if(substr($hidden_array[2], $key,1)=='1'){
					echo "(숨김)";
				}
				echo "<br>";
				}	
		...
		<tr>
			<td class="key">수상경력</td>
			<td colspan="3">
			<?php foreach ($M2['prize'] as $key => &$value) {
				echo $value;
				if(substr($hidden_array[3], $key,1)=='1'){
					echo "(숨김)";
				}
				echo "<br>";
				}	
		...
		<tr>
			<td class="key">강의경력</td>
			<td colspan="3">
			<?php foreach ($M2['teaching'] as $key => &$value) {
				echo $value;
				if(substr($hidden_array[4], $key,1)=='1'){
					echo "(숨김)";
				}
				echo "<br>";
				}	
		...
		<?php } ?>
		<tr>
			<td class="key">강의가능일시</td>
			<td colspan="3"><?php echo $M3['abletime']?></td>
		</tr>
		<tr>
			<td class="key">멘토의 한 마디</td>
			<td colspan="3"><?php echo $M3['talk']?></td>
		</tr>
		<tr>
			<td class="key">자기소개</td>
			<td colspan="3"><?php echo $M3['intro']?></td>
		</tr>
	<?php endif; ?>
		<tr>
			<td class="key" colspan="4"></td>
		</tr>
		<tr>
			<td class="key" colspan="4"><b>회원정보</b></td>
		</tr>
	<tr>
	<td class="key">이름</td>
	<td><?php echo $M['name']?></td>
