...
left join rb_dalkkum_group G on RQ.group_seq=G.uid left join rb_dalkkum_sclist SC on G.sc_seq=SC.uid',$sqlque,'RQ.uid as RUID, S.uid as SUID,RQ.group_seq as GUID, RQ.*,S.*, G.name as groupName,G.program_seq as program, SC.name as SCName, RQ.mentor_seq as mentor_seq',$sort,$orderby,$recnum,$p);
...
<div id="applylist">
	...
	<form name="requestsearchf" method="get" action="<?php echo $g['s']?>/mypage/">
		...
		<div class="category">
			...
			<select name="year2">
			<?php for($i=$date['year']+2;$i>2009;$i--):?><option value="<?php echo $i?>"<?php if($year2==$i):?> selected="selected"<?php endif?>><?php echo $i?>년</option><?php endfor?>
			</select>
			<select name="month2">
	...
	<tr>
		...
		<td><?php echo getDateFormat($R['date_start'],'Y.m.d')?></td>
		<td><?=$R['SCName']?></td>
		<td><?=$R['groupName']?></td>
		<td><?=number_format($temp['price'])."원"?></td>
	...
	<tr>
		<td class="sbj" colspan="8" style="height: 20px; line-height: 20px; text-align: center;">기타사항 : <?=$R['memo']?></td>
	</tr>
	<?php endwhile?> 
	<?php if(!$NUM):?>
	<tr>
	<td colspan="8">요청받은 내역이 없습니다.</td>
	</tr> 
	<?php else: ?>
	<tr>
		<td class="sbj" colspan="8" style="height: 20px; line-height: 20px;">강의료 정산 합계 : <?=number_format($sum_myprice)?>원</td>
	</tr>
	<?php endif?>
