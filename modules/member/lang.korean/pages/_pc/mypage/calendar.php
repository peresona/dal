<?php 
	...
	$_sql = "select T.uid, T.class_time, T.nows, G.name, G.class_date from rb_dalkkum_team as T, rb_dalkkum_group as G, rb_dalkkum_sclist as SC where T.group_seq = G.uid and G.sc_seq = SC.uid and T.mentor_seq=".$my['memberuid'];
	...
	while ($R = db_fetch_array($_result)) {
		$_tempdate = array('id' => $R['uid'], 'title' => $R['name'], 'start' => getDateFormat($R['class_date'],'Y-m-d'), 'url' => $R['']);
		array_push($printDate, $_tempdate);
	}
?><style>
