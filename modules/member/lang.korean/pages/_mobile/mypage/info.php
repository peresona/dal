...
<div id="pages_join">
	...
	else if($my['mentor_apply']) $level_ment = '<font color="blue">* 현재 멘토 신청이 접수되었으며<br> 운영팀이 확인 중 입니다.</font><br><a href="/?r=home&m=member&a=action&act=cancel_mentorApply" target=
	...
	<div class="join_cl">
		...
		<div class="cl"><?php $tel2=explode('-',$my['tel2'])?>
			<input type="password" name="now_pw" value="" maxlength="50" placeholder="현재 암호" class="d_form_underline center" style="width : 32%" /> 
			<input type="password" name="new_pw" value="" maxlength="50" placeholder="새 암호" class="d_form_underline center" style="width : 32%" /> 
	...
	<div class="join_cl">
		...
		<center>
			<?php if(strpos($g['url_host'], 'app.') && strpos($_SERVER['HTTP_USER_AGENT'],'Android')):?>
				<input type="button" value=" 사진 변경 " onclick="upload_photo('mypic_change','image/*','change_photo');">
			<?php else: ?>
...
<br>
	...
	<div id="plus_career" class="join_cl">
		...
		<div class="cl pr">
				...
				<?php if($key!=0):?><span onclick="cell_del(this)" class="icon cell_del cp"></span><?php endif; ?>
				<input type="text" name="m_career[]" placeholder="근무기간 ㅣ 회사명 및 부서 ㅣ 직위 ㅣ 담당업무" class="d_form_underline" value="<?=$value?>" />
		</div>
		<?php endforeach; ?>
	</div>
	...
	<div class="join_cl">
		<div class="cl">
				<div class="fl"><font class="black">6) 인터뷰에 들어갈 이미지를 첨부해주세요.<br>(없을 경우 기본이미지로 대체됩니다.)</font>
				</div>
		</div>
		<div class="cl">
				<center><?php if($_myMantor['i_pic']):?><img src="/_var/iphoto/360.<?=$_myMantor['i_pic']?>" style="max-height: 150px;" data-inhtml="iphoto_change" onclick="del_pic('iphoto_change','i_pic', '해당 이미지를 삭제하시겠습니까?');"><?php else: ?><img src="/_var/iphoto/default.png" style="max-height: 150px;" data-inhtml="iphoto_change"><?php endif;?>
				<?php if(strpos($g['url_host'], 'app.') && strpos($_SERVER['HTTP_USER_AGENT'],'Android')):?>
					<br><input type="button" value=" 사진 변경 " onclick="upload_photo('iphoto_change','image/*','change_photo');"></center>
				<?php else: ?>
	...
	<div class="join_cl">
		...
		<div class="cl">
			...
			<iframe width="100%" height="275" src="http://play.smartucc.kr/player.php?origin=<?=$_myMantor['media_key']?>&g=tag" frameborder="0" allowfullscreen data-inhtml="ivideo_change"></iframe><br><input type="button" class="btn" value="인터뷰 동영상 삭제 하기" style="width: 250px; height: 40px; line-height: 40px;" onclick="del_pic('ivideo_change','i_video', '해당 영상을 삭제하시겠습니까?');">
			<?php else: ?>
			<iframe width="0" height="0" frameborder="0" allowfullscreen data-inhtml="ivideo_change"></iframe><br>
			<?php endif; ?>
				
			<?php if(strpos($g['url_host'], 'app.') && strpos($_SERVER['HTTP_USER_AGENT'],'Android')):?>
				<br><input type="button" value=" 영상 변경 " onclick="upload_photo('ivideo_change','video/*','video_change');"></center>
			<?php else: ?>
	...
	<br>
	<div class="join_cl">
		<input type="checkbox" id="not_crime" name="not_crime" value="Y"<?php if($my['not_crime']=='Y'):?> checked="checked"<?php endif; ?>><label for="not_crime">성범죄조회 동의진행 및 인터뷰요청에 동의합니다. (선택) <br><font class="red">* 학교 강사 활동 시 필수 사항으로 동의 하지 않을 경우 활동이 제한됩니다</font></label>
	</div>
	<?php endif; ?>
	<div class="submitbox">
...
function video_change(arg){
		...
		if(str[1] && change_obj) change_obj.attr('src',str[1]); // 바꿀 이미지 대상 inhtml 값
		if(str[1] && change_obj) change_obj.attr('width','100%'); // 바꿀 이미지 대상 inhtml 값
		if(str[1] && change_obj) change_obj.attr('height','275'); // 바꿀 이미지 대상 inhtml 값
...
function change_photo(arg){
		...
		if(str[1] && change_obj) change_obj.attr('src',moresrc + str[1]+'?number='+Math.random()*1000); // 바꿀 이미지 대상 inhtml 값
		if(str[2]) alert(str[2]);
}

function upload_photo(imgName,type,succFn){
	var param = {
		desUrl : rooturl + "/?r=home&m=member&a=upload_mphoto&gcode=<?=$my['id']?>&ucode=<?=$my['memberuid']?>&imgName="+imgName, // 파일 전송될 주소
		type : type,
		succFn : succFn // 성공 전달 함수 이름
	};
	if(type == 'video/*') param['desUrl'] = rooturl + "/?r=home&m=member&a=upload_mvideo&gcode=<?=$my['id']?>&ucode=<?=$my['memberuid']?>&imgName="+imgName; // 파일 전송될 주소
			...
			success: function(response) {
				...
				if(results.code == '100'){
					...
					if(obj == 'mypic_change'){
						...
						ctarget.attr('src','/_var/iphoto/default.png');
					}else if(obj == 'ivideo_change'){
						ctarget.attr('src','');
						ctarget.attr('width','0');
						ctarget.attr('height','0');
					}
				}
