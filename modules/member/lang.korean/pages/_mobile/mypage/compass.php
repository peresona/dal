...
<div id="compass">
	...
	<h2><?php if($my['mentor_confirm']=='Y' && $my['mentor_job']):?>멘토 진로나침반(<?=getJobName($my['mentor_job'])?>)<?php else:?>나의 진로 Q&amp;A<?php endif;?></h2>
	<div class="info">
	...
	<tr onclick="location.href='/jblog/kin/?job=<?=$R['job_seq']?>&uid=<?=$R['uid']?>'">
	<td><?=$NUM-((($p-1)*$recnum)+$_rec++)?></td>
	<td class="cat"><?=$R['jobName']?></td>
	<td class="sbj">
		<font color="#000"><?=$R['subject']?></font>
	...
	<?php if(!$NUM):?>
	<tr>
		<td align="center" colspan="5">출력할 질문이 없습니다.</td>
	</tr> 
	<?php endif?>
