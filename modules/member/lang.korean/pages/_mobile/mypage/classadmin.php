		...
		<td colspan="5">
			<table style="background-color: #eee; ">
				...
				<tr>
					...
					<tr class="intable">
						<td colspan="6">
							<table>
								<tr>
								<?php 
								$scoreName = array('강의내용','매너','수업분위기');

								for ($i=1; $i <= 3 ; $i++) :?>
									<td><b><?=$scoreName[$i-1]?></b><br>
								...
								<?php endfor; ?>
									...
									<?php if($PL['score1'] && $PL['score2'] && $PL['score3'] && $PL['score4']):?>
										<input type="button" value="리셋" onclick="resetScore('<?=$PL['memberuid']?>','<?=$R['uid']?>')" title="점수를 초기화합니다." data-togglebtn="ts_<?=$PL['memberuid']?>_<?=$R['uid']?>">
									<?php else:?>
										<input type="button" value="적용" onclick="saveScore('<?=$PL['memberuid']?>','<?=$R['uid']?>')" title="점수를 적용합니다." data-togglebtn="ts_<?=$PL['memberuid']?>_<?=$R['uid']?>">
									<?php endif; ?>
									</td>
								</tr>
							</table>
					...
					<?php if(!$tmpnum):?>
						<tr>
							<td colspan="6" class="center">출력할 멘토가 없습니다.</td>
						</tr>
					<?php endif; ?>
			</table>
		</td>
