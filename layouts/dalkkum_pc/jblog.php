<?php 
	...
	elseif($job && $uid && $c && !$mod){
		...
		if($_temp['parentmbr']!='0') {
			...
			getLink('/'.$c.'/?job='.$job.'&uid='.$temp2['uid'].($mode?'&mode='.$mode:''),'','','','replace');
		}
	}
	$JD = getDbData('rb_dalkkum_job','uid='.$job,'*');
	$numMenti = getDbRows('rb_dalkkum_myjob','job_seq='.$job);
	if(!$JD['uid'] && $mod != 'write') getLink('/','','정상적인 접근이 아닙니다.','','replace');
...
<?php if($job):?>
	<div id="personal_head" class="cl" style="background: url('/files/_etc/jblog/<?=($JD["bg_pc"]?$JD["bg_pc"]:'default.jpg')?>') top center no-repeat; background-size: cover;">
		<div class="inner_wrap cl center">
			<span class="center icon d_job_bg cl active" onclick="popup_jobs('<?=$JD["uid"]?>');"><img src="/files/_etc/job/<?=($JD["img"]?$JD["img"]:'default.png')?>" alt=""></span>
			<div class="cl" style="margin: 10px 0 25px 0; height: 60px;" onclick="popup_jobs('<?=$JD["uid"]?>');"><h1><?=$JD["name"]?></h1></div>
			<div class="cl">
