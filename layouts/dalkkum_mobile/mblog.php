<?php 
	...
	if($PD['mentor_confirm']!='Y') getLink('/','','해당 회원은 멘토 회원이 아닙니다.','');
...
<section>
	<div id="personal_head" class="cl">
		<div class="inner_wrap cl">
			<div class="fl" style="margin-right:25px;"><div style="width: 200px; height: 200px; border-radius: 100px; background: url(/_var/simbol/180.<?php if($PD['photo']):?><?=$PD['photo']?><?php else: ?>default.jpg<?php endif ?>) top center; background-size: cover; border : solid 3px #FEC55A; "></div></div>
			<div class="fl" style="margin-top: 20px;">
				<div class="cl smallinfo" style="margin-bottom: 10px;">멘티 <b><?=$numMenti?></b> | 멘토링 <b><?=($numMentoring+$numMentoring2)?></b></div>
				<div class="cl" style="margin:20px 0;"><?=$PD['name']?> 멘토 (<a href="/jblog/?job=<?=$PD['mentor_job']?>" style="color:#FFF"><?=getJobName($PD['mentor_job'])?></a>)</div>
